          ...
          <div class="collapse navbar-collapse justify-content-md-end me-5 pe-5" id="navbarsExample10">
            <ul class="navbar-nav fs-5 primary">
              ...
              <li class="nav-item dropdown px-2">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdown10">
                  <li><a class="dropdown-item" href="{{url('/scolarship_form')}}">Bursë</a></li>
...
<div class="container mt-5 pt-5" id="bck-image">
  <h2 class="mt-5 pt-5">Lista e pjestareve te konviktit</h2>
  <table class="table table-condensed">
    ...
    <tbody>
        @foreach ($dorm_members as $dorm_member)
            <tr>
                <td>{{ $dorm_member['name'] }}</td>
                <td>{{ $dorm_member['surname'] }}</td>
                <td>{{ $dorm_member['email'] }}</td>
                <td>{{ $dorm_member['phone'] }}</td>
                <td>{{ $dorm_member['school'] }}</td>
                <td>{{ $dorm_member['class'] }}</td>
                <td>{{ $dorm_member['scolarship'] }}</td>
                <td>{{ $dorm_member['payment'] }}</td>
                <td>
                    <a class="btn btn-danger" href="{{ 'delete/'.$dorm_member['id'] }}" role="button">Delete</a>
                    <a class="btn btn-warning" href="{{ 'editMember/'.$dorm_member['id'] }}" role="button">Edit</a>
                </td>
            </tr> 
        @endforeach
    </tbody>
  </table>
  <a class="btn btn-primary" href="{{ url('/dormMembers_form') }}" role="button">Shto nje pjestar</a>
</div>
